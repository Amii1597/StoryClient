  ...
  useEffect(() => {
    (async () => {
      ...
      let stories = await axios.get(
        `https://swiptory22.onrender.com/user/story/${user}`
      );
...
async function getSelectedStories(category) {
  try {
    let response;
    if (category == "all")
      response = await axios.get("https://swiptory22.onrender.com/story/all");
    else
      response = await axios.get(
        `https://swiptory22.onrender.com/story/all?category=${category}`
      );
...
async function getBookMarkedStories () {
  const user = localStorage.getItem('user');
  try {
    let bookmarks = await axios.get(`https://swiptory22.onrender.com/user/bookmarks/${user}`, {
      headers: {
    ...
    for (const item of bookmarks) {
      ...
      try {
        const response = await axios.get(`https://swiptory22.onrender.com/story/${storyID}`);
