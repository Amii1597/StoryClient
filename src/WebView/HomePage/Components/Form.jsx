  ...
  const handleSubmit = async (e, formData) => {
    ...
    if (props.isSignUp) {
      const response = await axios.post(
        "https://swiptory22.onrender.com/register",
        formData,
        {
          headers: {
      ...
      else setResponse(`${response.data.Success}. User successfully created.`);
    }
    if (props.isLogIn) {
      const response = await axios.post(
        "https://swiptory22.onrender.com/login",
        formData,
        {
          headers: {
            "content-type": "application/x-www-form-urlencoded",
...
function signUp(formData, setFormData, handleSubmit, response) {
  return (
    <>
      <form
        action="https://swiptory22.onrender.com/register"
        method="POST"
        onChange={(e) =>
          setFormData({ ...formData, [e.target.name]: e.target.value })
        }
        onSubmit={(e) => handleSubmit(e, formData)}>
      {/* <div  onClick={(props)=>props.setModal(false)}>  */}
...
function logIn(formData, setFormData, handleSubmit, response) {
  return (
    <>
      <form
        action="https://swiptory22.onrender.com/login"
        method="post"
        onChange={(e) =>
          setFormData({ ...formData, [e.target.name]: e.target.value })
        }
